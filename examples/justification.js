...
function makeTextPanel(contentDirection) {
	...
	for (let i = 0; i < justificationLegend.length; i++) {
		...
		const panel = buildJustifiedPanel(
			id,
			color,
			contentDirection === "column" ? "row" : "column",
		);
...
setInterval(() => {
	...
	for (let i = 1; i < justifyInRow.children.length; i++) {
		justifyInRow.children[i].set({
			contentDirection: isInverted ? "row-reverse" : "row",
		});
	}

	for (let i = 1; i < justifyInColumn.children.length; i++) {
		justifyInColumn.children[i].set({
			contentDirection: isInverted ? "column-reverse" : "column",
		});
	}
}, 2500);
